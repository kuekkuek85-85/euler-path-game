...
| B02 도전 회로 | 매번 새 도형(무한 모드) | **고정 미션 5개 (B02~B06, 간선 12·13·14·16·18)** | 2026-09-01 작성자 결정. 도형이 고정되어야 "최단 시간" 순위를 서로 비교할 수 있다. 도형은 `lib/generator.ts`로 뽑아 `stages.json`에 심었다 |
...
| 두붓 그리기 | 없음 | **C01~C04 신설 (`maxStrokes: 2`)** | 2026-09-01 작성자 제안. S11에서 "4 ÷ 2 = 2붓"을 판정만 하고 끝나던 것을 실제로 그려 보게 한다. C03·C04는 S11·B01과 같은 도형이라 판정 → 실행으로 이어진다 |
| 도전 경로 | 없음 | **D01~D03 신설 (홀수점 2개, 간선 13·15·17)** | 2026-09-01 작성자 요청. 보너스가 회로형뿐이라 "시작점을 골라야 한다"는 2단계의 핵심 기능이 도전 구간에 없었다 |
