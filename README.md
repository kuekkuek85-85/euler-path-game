...
4. 어디서 시작할지 모르겠으면 **시작점 힌트**를 3단계까지 받을 수 있다.
   단계마다 후보가 좁아지고, 대신 점수가 깎인다.
5. 노트북은 마우스 버튼을 누른 채 끌면 된다.

#### 시작점 힌트 (3단계)

| 단계 | 알려주는 것 | 감점 |
|---|---|---|
| 1단계 | 시작점이 들어 있는 **넓은 범위**를 반짝인다 (점의 약 65%) | -20 |
| 2단계 | 범위를 **더 좁혀서** 반짝인다 (약 40%) | -40 |
| 3단계 | 시작점 후보 **2개만** 콕 집어 반짝인다 | -80 |

- 단계는 좁아지기만 한다 — 3단계 ⊆ 2단계 ⊆ 1단계라, 앞 단계에 있던 답이 뒤에서 사라지지 않는다.
- 홀수점이 2개인 도형이면 **3단계가 곧 정답**(그 두 홀수점)이다.
- 홀수점이 없는 도형은 어디서 시작해도 되므로, 3단계의 두 점은 "이 둘만 된다"가 아니라
  **골라 보라는 예시**다. 안내 문구가 그 차이를 말해 준다.
- 누르면 바로 깎이지 않는다. **얼마가 깎이는지 보여주는 확인창**에서 "받을게요"를 눌러야 한다.
- 다시 그려도 이미 받은 힌트는 남아 있다 — 점수를 주고 산 것을 두 번 사게 하지 않는다.

> 다시하기 버튼과 홀수점 보기는 학생이 스스로 고민하도록 **감춰 두었다.**
> 홀수점 보기는 교사가 "홀수점 보기 열기"를 켰을 때만 나타난다.
> 실패 복구는 붓을 뗐을 때 캔버스에 뜨는 **다시 그리기**로 한다(3초 뒤 저절로도 된다).
