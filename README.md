...
Firebase 설정 없이도 **모든 스테이지가 플레이된다**(기록은 이 기기에만 저장).
...
1. QR/짧은 URL 접속 → 학번 5자리(예: `10307`)와 이름 입력 → 시작.
   같은 학번·이름으로 다시 들어오면 **이전 기록을 그대로 이어서** 한다.
   첫 화면 맨 위의 **이어서 하기** 버튼과 "지금 여기" 표시가 다음에 할 미션을 가리킨다.
1. **레벨 6개 카드** 중 하나를 골라 → 그 레벨의 **미션 10개**에서 고른다.
   미션 화면의 `← 레벨 선택`(또는 휴대폰 뒤로 가기)으로 되돌아온다.
2. 시작할 점을 **누른 채** 이어진 점으로 끈다.
   **도중에 손을 떼면 한붓그리기가 아니므로 실패**하고 처음부터 다시 그린다.
3. 막히거나 실패하면 캔버스의 **다시 그리기** 버튼으로 처음부터.
4. 노트북은 마우스 버튼을 누른 채 끌면 된다.

> 힌트·다시하기 버튼과 홀수점 보기는 학생이 스스로 고민하도록 **감춰 두었다.**
> 홀수점 보기는 교사가 "홀수점 보기 열기"를 켰을 때만 나타난다.

미션은 레벨당 10개씩이다 — **1~6레벨, 레벨당 10개씩 모두 60개(L1-01 ~ L6-10)** 이고 이게 전부다.
앞 스테이지를 깨야 다음이 열린다. 60개 전부 작성자가 원본 그림과 대조해 검수를
마쳤고(2026-09-06), 모두 홀수점이 0개 또는 2개라 한 붓에 그려진다.
...
| 이름 마스킹 | 켜면 `김민수` → `김O수` (**기본 꺼짐** — 대시보드에 전체 이름이 보인다) |
| 홀수점 보기 열기 | 기본은 꺼짐(학생 화면에 버튼이 아예 없다). 켜면 홀수점 보기 버튼이 나타난다. **수업 흐름상 "정리 1"에서 켠다** |
| 스테이지 활성화 | 진도에 맞춰 2레벨을 잠가 둘 수 있다 |
...
| 전개 1 | 12분 | L1-01~L1-06. "왜 어떤 건 되고 어떤 건 안 되지?" |
| 정리 1 | 5분 | 홀수점 개념 설명 → 교사 모드에서 **홀수점 보기 열기** 켜기 |
| 전개 2 | 12분 | L1-07~L1-10. 시작점을 골라야 함을 체험 |
| 전개 3 | 8분 | L2-01~. 선이 많아져도 규칙은 같다 |
| (여유) | — | 먼저 끝낸 학생은 3레벨(L3-01~L3-10)로 |
...
- 이미지에서 옮긴 점·선 개수와 홀수점 개수가 대조표와 일치하는지
- 점끼리 손가락 하나로 헷갈릴 만큼 붙어 있지는 않은지

## 5. 디렉터리 구조

```
src/
  data/stages.json      스테이지 데이터 (레벨당 10개)
  data/stages.ts        로더 + 개발 모드 자가 검사
  lib/
    graph.ts            차수·홀수점·연결성·오일러 판정·Hierholzer·힌트
    geometry.ts         간선 SVG 경로 (직선 / 2차 베지어)
    generator.ts        도전 회로·경로 도형 생성 — 설계 시점 도구
    scoring.ts          점수·별점 산식
  ...
  components/
    ...
    decor/              배경 일러스트 (3레벨 판별 미션용, 현재 미사용)
