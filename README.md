...
Firebase 설정 없이도 **20스테이지 전부 플레이된다**(기록은 이 기기에만 저장).
...
미션은 **1레벨 10개(L1-01~L1-10)** 와 **2레벨 10개(L2-01~L2-10)**, 모두 20개다.
앞 스테이지를 깨야 다음이 열린다. 3레벨은 준비 중이다.
...
```
src/
  data/stages.json      스테이지 데이터 (1레벨 10 + 2레벨 10)
