...
**두붓 그리기(C01~C04)** 는 홀수점이 4개라 한 붓으로는 못 그리는 도형이다.
붓을 **한 번만** 떼도 되고, 뗀 뒤에는 남은 선이 있는 **아무 점에서나** 다시 시작할 수 있다.
두 번 떼면 실패다. 화면 위쪽에 "붓 1 / 2"로 몇 번째 붓인지 표시된다.
...
| (여유) | — | 먼저 끝낸 학생은 보너스 → 도전 경로 → **두붓 그리기**로 |
...
```
src/
  data/stages.json      스테이지 데이터 (본편 12 + 보너스 6 + 도전 경로 3 + 두붓 4)
